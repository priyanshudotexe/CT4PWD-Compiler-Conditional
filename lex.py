...
def _normalize_qr_text(raw_text: str) -> str:
    """Lowercase and strip non-alphanumeric characters (keep spaces just in case)."""
    if raw_text is None:
        return ""
    s = raw_text.strip().lower()
    # Keep only letters, digits and spaces (drop punctuation)
    s = re.sub(r'[^a-z0-9 ]+', '', s)
    # If there's extra whitespace, collapse it
...
def _classify_qr_text(text, x, y, w, h, image):
    ...
    # Fallback: if it's a simple token, treat as label (safe fallback)
...
def detect_qr_and_blocks(image):
    ...
    # Sort left-to-right (pyzbar returns rect with .left)
